...
try {
    if (file_exists(__DIR__ . '/v2/src/Config/Env.php')) {
        ...
        require_once __DIR__ . '/v2/src/Repositories/GradientRepository.php';

        $repo = new \ColorMagic\Repositories\GradientRepository();
        ...
        if ($limit > 0 || $style !== '' || $type !== '' || $q !== '') {
            $result = $repo->search($q, $style !== '' ? $style : null, $type !== '' ? $type : null, $page, $limit > 0 ? $limit : 50);
            if ($limit > 0) {
...
$dataFile = __DIR__ . '/data/gradients.json';
if (!is_file($dataFile)) {
    $dataFile = __DIR__ . '/gradients.json';
}
if (!is_file($dataFile)) {
    $dataFile = dirname(__DIR__) . '/data/gradients.json';
}

if (!is_file($dataFile)) {
    http_response_code(500);
    echo json_encode(['status' => 'error', 'message' => 'Gradients dataset not found']);
    exit;
}
...
if ($id !== '') {
    ...
    http_response_code(404);
    echo json_encode(['status' => 'error', 'message' => 'Gradient not found']);
    exit;
}

$filtered = array_values(array_filter($data, static function ($item) use ($style, $type, $q) {
    if (!is_array($item)) return false;
    if ($style !== '' && strtolower((string)($item['style'] ?? '')) !== $style) return false;
    if ($type !== '' && strtolower((string)($item['type'] ?? '')) !== $type) return false;
    if ($q !== '') {
