...
<body class="bg-background-light dark:bg-background-dark font-display text-slate-900 dark:text-white min-h-screen">
    ...
    <main class="w-full max-w-7xl mx-auto pt-16 pb-8 px-6 md:px-8 flex flex-col gap-6">
        <!-- Hero -->
        <div
            class="relative overflow-hidden rounded-2xl bg-gradient-to-br from-emerald-500/10 via-pink-50 to-amber-500/10 dark:from-emerald-500/20 dark:via-slate-900 dark:to-amber-500/20 p-7 md:p-10 border border-emerald-100 dark:border-slate-800">
            <div
                class="absolute w-56 h-56 bg-emerald-500/15 rounded-full blur-[70px] top-[-30px] right-[-30px] pointer-events-none">
            </div>
            <div class="relative z-10">
                <div
                    class="inline-flex items-center gap-2 px-3 py-1.5 bg-white/70 dark:bg-slate-800/70 backdrop-blur-sm rounded-full text-xs font-bold text-emerald-600 dark:text-emerald-400 mb-3 border border-emerald-200/50 dark:border-emerald-700/50">
                ...
                <p class="text-slate-600 dark:text-slate-300 text-base max-w-lg">
                    Find the exact hex codes, RGB values, and color palettes used by the world's most recognizable
                    brands. Click any color to explore its full details.
                </p>
            </div>
        </div>
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            ...
            foreach ($brands as $brand):
                ?>
                <div
                    class="bg-white dark:bg-slate-900 rounded-2xl p-5 border border-slate-200 dark:border-slate-800 hover:shadow-lg transition-shadow">
                    ...
                    <div class="space-y-2">
                        <?php foreach ($brand['colors'] as $color):
                            ...
                            <a href="<?= $base ?>/color/<?= $colorSlug ?>/" class="flex items-center gap-3 group">
                                <span
                                    class="w-10 h-10 rounded-lg shadow-sm shrink-0 border border-slate-100 dark:border-slate-700"
                                    style="background:#<?= $hexUpper ?>"></span>
                                ...
                                <i
                                    class="bi bi-arrow-up-right text-slate-300 dark:text-slate-600 group-hover:text-primary transition-colors"></i>
                            </a>
                        <?php endforeach; ?>
                    </div>
                </div>
        ...
        <div class="mb-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div class="bg-white dark:bg-slate-900 rounded-2xl p-6 border border-slate-200 dark:border-slate-800">
                    ...
                    <p class="text-sm text-slate-600 dark:text-slate-400">Most tech brands use blue for trust (Google
                        #4285F4, Meta #0668E1, LinkedIn #0A66C2). Red is popular for energy and excitement (Netflix
                        #E50914, YouTube #FF0000, Adobe #FF0000). Green represents growth (Spotify #1DB954, Shopify
                        #7AB55C). Black conveys premium quality (Apple #000000, Uber #000000).</p>
                </div>
                <div class="bg-white dark:bg-slate-900 rounded-2xl p-6 border border-slate-200 dark:border-slate-800">
                    <h3 class="font-bold text-base mb-2">Where to find brand color codes?</h3>
                    <p class="text-sm text-slate-600 dark:text-slate-400">This page provides a curated collection of
                        brand color hex codes. You can also find official brand guidelines on each company's press/brand
                        page. Click any color above to see its full RGB, HSL, and palette details on its dedicated color
                        page.</p>
                </div>
                <div class="bg-white dark:bg-slate-900 rounded-2xl p-6 border border-slate-200 dark:border-slate-800">
                    <h3 class="font-bold text-base mb-2">What is Google's brand color?</h3>
                    <p class="text-sm text-slate-600 dark:text-slate-400">Google uses four brand colors: Blue (#4285F4),
                        Red (#EA4335), Yellow (#FBBC05), and Green (#34A853). These are used across their logo,
                        products, and marketing materials. The blue (#4285F4) is the most recognizable and is used for
                        the Google "G" logomark.</p>
                </div>
                <div class="bg-white dark:bg-slate-900 rounded-2xl p-6 border border-slate-200 dark:border-slate-800">
                    <h3 class="font-bold text-base mb-2">How to use brand colors in my design?</h3>
                    <p class="text-sm text-slate-600 dark:text-slate-400">Brand colors can inspire your own palette. Use
                        them as a starting point — pick a primary color similar to the brand you admire, then use our <a
                            href="<?= $base ?>/generate-palette" class="text-primary hover:underline">Palette
                            Generator</a> to create harmonious schemes. Be careful not to copy trademarked colors
                        directly for commercial use.</p>
                </div>
                <div class="bg-white dark:bg-slate-900 rounded-2xl p-6 border border-slate-200 dark:border-slate-800">
                    <h3 class="font-bold text-base mb-2">Are brand colors trademarked?</h3>
                    <p class="text-sm text-slate-600 dark:text-slate-400">Yes, brand colors can be trademarked in
                        certain contexts. While you can use hex codes for personal projects and inspiration, using a
                        competitor's exact brand colors in commercial work may cause legal issues. Always consult brand
                        guidelines and legal counsel for commercial applications.</p>
                </div>
            </div>
        </div>
