...
function getHueFamily(int $h, int $s, int $l): string {
    if ($s < 10) return 'Neutrals';
    if ($h < 15 || $h >= 345) return 'Dark Reds';
    if ($h < 45) return 'Dark Oranges';
    if ($h < 70) return 'Dark Yellows';
    if ($h < 160) return 'Dark Greens';
    if ($h < 200) return 'Dark Cyans';
    if ($h < 260) return 'Dark Blues';
    if ($h < 290) return 'Dark Purples';
    return 'Dark Pinks';
}

$darkColors = [];
foreach ($colorNames as $entry) {
    if (!is_array($entry) || !isset($entry['hex'])) continue;
    $hex = $entry['hex'];
...
<body class="bg-background-light dark:bg-background-dark font-display text-slate-900 dark:text-white min-h-screen">
    ...
    <main class="w-full max-w-7xl mx-auto pt-16 pb-8 px-6 md:px-8 flex flex-col gap-6">
        <!-- Hero -->
        <div class="relative overflow-hidden rounded-2xl bg-gradient-to-br from-indigo-500/10 via-slate-100 to-slate-200 dark:from-indigo-500/20 dark:via-slate-900 dark:to-slate-800 p-7 md:p-10 border border-indigo-100 dark:border-slate-800">
            <div class="absolute w-56 h-56 bg-indigo-500/15 rounded-full blur-[70px] top-[-30px] right-[-30px] pointer-events-none"></div>
            <div class="relative z-10">
                <div class="inline-flex items-center gap-2 px-3 py-1.5 bg-white/70 dark:bg-slate-800/70 backdrop-blur-sm rounded-full text-xs font-bold text-indigo-600 dark:text-indigo-400 mb-3 border border-indigo-200/50 dark:border-indigo-700/50">
                ...
                <p class="text-slate-600 dark:text-slate-300 text-base max-w-lg">
                    Discover the names of deep, dark color shades. Browse <?= count($darkColors) ?>+ dark colors organized by hue family — from dark reds and blues to deep purples and midnight tones.
                </p>
            </div>
        </div>
        ...
        <div class="mb-6">
            ...
            <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-3">
                ...
                <a href="<?= $base ?>/color/<?= htmlspecialchars($color['slug'] ?? strtolower($color['hex'])) ?>/"
                    ...
                    <span class="w-full h-16 rounded-lg shadow-sm" style="background:#<?= htmlspecialchars($color['hex']) ?>"></span>
                    <p class="font-bold text-xs text-center leading-tight"><?= htmlspecialchars($color['name']) ?></p>
                    <p class="text-[10px] font-mono text-slate-400">#<?= htmlspecialchars(strtoupper($color['hex'])) ?></p>
                </a>
                <?php endforeach; ?>
            </div>
        </div>
        ...
        <div class="mb-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div class="bg-white dark:bg-slate-900 rounded-2xl p-6 border border-slate-200 dark:border-slate-800">
                    ...
                    <p class="text-sm text-slate-600 dark:text-slate-400">A color is considered dark when its lightness value in the HSL color model is below 40%. This means it reflects less light and appears deeper or closer to black. Dark colors can still be vibrant — they just have low lightness regardless of their saturation or hue.</p>
                </div>
                <div class="bg-white dark:bg-slate-900 rounded-2xl p-6 border border-slate-200 dark:border-slate-800">
                    <h3 class="font-bold text-base mb-2">What are popular dark color names?</h3>
                    <p class="text-sm text-slate-600 dark:text-slate-400">Some of the most popular dark colors include Midnight Blue (#191970), Navy (#000080), Dark Slate Gray (#2F4F4F), Burgundy (#800020), Dark Green (#006400), and Indigo (#4B0082). These are widely used in web design, branding, and interior design.</p>
                </div>
                <div class="bg-white dark:bg-slate-900 rounded-2xl p-6 border border-slate-200 dark:border-slate-800">
                    <h3 class="font-bold text-base mb-2">Best dark colors for web design?</h3>
                    <p class="text-sm text-slate-600 dark:text-slate-400">Dark colors work excellently as backgrounds, text colors, and accent shades. Popular choices include dark navy (#1E293B) for backgrounds, charcoal (#334155) for text, and deep jewel tones like Dark Emerald (#006400) or Burgundy (#800020) for accents and CTAs.</p>
                </div>
                <div class="bg-white dark:bg-slate-900 rounded-2xl p-6 border border-slate-200 dark:border-slate-800">
                    <h3 class="font-bold text-base mb-2">What is the darkest color besides black?</h3>
                    <p class="text-sm text-slate-600 dark:text-slate-400">The darkest named colors besides pure black (#000000) include Rich Black (#000814), Dark Navy (#000C24), and Vampire Black (#080808). In practice, Vantablack — a synthetic material — absorbs 99.965% of visible light, making it the darkest substance known.</p>
                </div>
                <div class="bg-white dark:bg-slate-900 rounded-2xl p-6 border border-slate-200 dark:border-slate-800">
                    <h3 class="font-bold text-base mb-2">How to find dark shade hex codes?</h3>
                    <p class="text-sm text-slate-600 dark:text-slate-400">Browse the gallery above — every color shown has a lightness value below 40%, making them all genuinely dark. Click any swatch to see its full details, or use our <a href="<?= $base ?>/find-color" class="text-primary hover:underline">Find Color tool</a> to check if any hex code qualifies as dark.</p>
                </div>
            </div>
        </div>
